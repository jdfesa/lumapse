...
- **Refrescos vecinos de escrituras confirmadas:** crear una materia/sección y editar o eliminar una fecha ya no se informan como fallidos si SQLite confirmó la mutación y solo falla la lectura secundaria. El aviso persistente recupera árbol/conteos o próximas fechas sin repetir `INSERT`, `UPDATE` ni `DELETE`; los fallos reales conservan el contrato anterior y AUD-006 protege caches ante respuestas obsoletas. Implementado en PR #19 con SQLite/UI y gate canónico; prueba Android y aprobación del autor pendientes. [Plan, evidencia y límites](docs/gestion/plan-refrescos-vecinos-2026-09-16.md).

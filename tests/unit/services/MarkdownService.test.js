...
describe('preprocessNoteLinks()', () => {
  it('convierte [[Título]] en link Markdown con protocolo lumapse://', () => {
    const result = MarkdownService.preprocessNoteLinks('Ver [[Mi Nota]]')
    expect(result).toContain('[🔗 Mi Nota](lumapse://note/')
  })

  it('retorna el mismo texto si no hay [[links]]', () => {
    const input = 'Texto sin links internos'
    expect(MarkdownService.preprocessNoteLinks(input)).toBe(input)
  })

  it('maneja títulos con espacios y acentos', () => {
    const result = MarkdownService.preprocessNoteLinks('[[Programación Avanzada]]')
    expect(result).toContain('🔗 Programación Avanzada')
    expect(result).toContain('lumapse://note/')
  })

  it('maneja múltiples links en el mismo texto', () => {
    const result = MarkdownService.preprocessNoteLinks('Ver [[Nota A]] y [[Nota B]]')
    expect(result).toContain('🔗 Nota A')
    expect(result).toContain('🔗 Nota B')
  })

  it('no convierte corchetes simples [texto]', () => {
    const input = 'Un [texto] entre corchetes simples'
    expect(MarkdownService.preprocessNoteLinks(input)).toBe(input)
  })

  it('retorna null/undefined sin error', () => {
    expect(MarkdownService.preprocessNoteLinks(null)).toBeNull()
    expect(MarkdownService.preprocessNoteLinks(undefined)).toBeUndefined()
  })

  it('codifica caracteres especiales en la URL', () => {
    const result = MarkdownService.preprocessNoteLinks('[[Nota con & y #]]')
    expect(result).toContain(encodeURIComponent('Nota con & y #'))
  })
})

describe.skip('Link Lumapse — renderizado completo (DESACTIVADO)', () => {
  it('[[Título]] se renderiza como <a> con clase note-link', () => {
    const html = MarkdownService.renderMarkdown('Texto con [[Mi Nota]] aquí')
    expect(html).toContain('<a')
    expect(html).toContain('lumapse://note/')
    expect(html).toContain('class="note-link"')
    expect(html).toContain('🔗 Mi Nota')
  })

  it('links lumapse:// no tienen target="_blank"', () => {
    const html = MarkdownService.renderMarkdown('[[Nota interna]]')
    // El link NO debe tener target="_blank" (es navegación interna)
    const match = html.match(/<a[^>]*lumapse:\/\/[^>]*>/)
    if (match) {
      expect(match[0]).not.toContain('target="_blank"')
    }
  })

  it('links lumapse:// sobreviven DOMPurify', () => {
    const html = MarkdownService.renderMarkdown('[[Test de seguridad]]')
    expect(html).toContain('lumapse://note/')
  })
})
